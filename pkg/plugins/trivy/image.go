...
func GetPodSpecForStandaloneMode(ctx trivyoperator.PluginContext,
	...
	for _, c := range containersSpec {
		...
		env := []corev1.EnvVar{
			...
			constructEnvVarSourceFromConfigMap("TRIVY_SKIP_VERSION_CHECK", trivyConfigName, keyTrivySkipVersionCheck),
			constructEnvVarSourceFromConfigMap("TRIVY_DISABLE_TELEMETRY", trivyConfigName, keyTrivyDisableTelemetry),
